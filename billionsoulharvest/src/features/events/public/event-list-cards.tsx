...
export function EventListCards({ columns, maxItems, statusFilter, cardBg, cardBorder, titleColor, textColor, gap }: Props) {
    ...
    return (
      <div style={{ display: "grid", gridTemplateColumns: `repeat(${columns}, 1fr)`, gap: `${gap}px` }}>
        {Array.from({ length: Math.min(maxItems, 3) }).map((_, i) => (
          <div key={i} style={{ background: cardBg, border: `1px solid ${cardBorder}`, borderRadius: "16px", overflow: "hidden" }}>
            <div style={{ height: "140px", background: "linear-gradient(135deg, #1e3a5f, #0f2744)" }} />
            <div style={{ padding: "20px" }}>
              <div style={{ height: "14px", width: "60%", background: "rgba(255,255,255,0.1)", borderRadius: "4px", marginBottom: "12px" }} />
              <div style={{ height: "12px", width: "80%", background: "rgba(255,255,255,0.05)", borderRadius: "4px" }} />
            </div>
          </div>
        ))}
      </div>
