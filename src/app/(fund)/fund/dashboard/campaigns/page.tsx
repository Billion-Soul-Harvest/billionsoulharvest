    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Link
          ...
          className="inline-flex items-center px-4 py-2 text-sm font-medium text-white bg-cyan-600 hover:bg-cyan-700 rounded-lg"
        >
          New Campaign
        </Link>
      </div>
        ...
        {typedCampaigns.map((c) => (
          <div key={c.id} className="p-4 flex items-center gap-4">
            <div className="flex-1 min-w-0">
              <Link href={`/fund/dashboard/campaigns/${c.id}/edit`} className="font-medium text-gray-900 hover:text-cyan-700 truncate block">
                {c.title}
              </Link>
