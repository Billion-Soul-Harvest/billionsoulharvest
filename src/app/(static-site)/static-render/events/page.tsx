...
export const metadata: Metadata = {
  ...
  description:
    "Browse upcoming and past Billion Soul Harvest events and summits.",
};
...
export default async function StaticEventsListPage() {
      ...
      <section className="py-20 sm:py-28">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <p className="text-lg text-gray-300/80 max-w-2xl mx-auto leading-relaxed">
            Global Harvest Summits and regional gatherings bringing church
            leaders together for worship, equipping, and partnership.
          </p>
        </div>
      </section>
          ...
          {events && events.length > 0 ? (
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              {events.map((event) => {
                ...
                const isClickable =
                  hasExternalUrl || !!event.page_content;
                const href = hasExternalUrl
                  ? event.external_url
                  : `/events/${event.slug}`;
                const cardClass = `bg-white/5 border border-white/10 rounded-2xl overflow-hidden ${isClickable ? "group hover:border-[#29BDD6]/30" : ""} transition-all`;
                  ...
                  <>
                    <div className="h-40 bg-gradient-to-br from-[#1e3a5f] to-[#0f2744] flex items-center justify-center">
                      {event.banner_url ? (
                        ...
                        <div className="text-[#29BDD6]/30 text-4xl font-bold">
                          BSH
                        </div>
                      )}
                    </div>
                    ...
                    <div className="p-5">
                      ...
                      <h3
                        className={`font-[family-name:var(--font-heading)] text-lg font-bold text-white ${isClickable ? "group-hover:text-[#29BDD6]" : ""} transition-colors mb-2`}
                      >
                        {event.title}
                      </h3>
