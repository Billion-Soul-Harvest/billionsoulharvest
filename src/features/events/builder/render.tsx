    ...
    case "CraftEventTitle": {
      ...
      return (
        <div
          ...
          className="font-[family-name:var(--font-jakarta)] font-bold tracking-[-0.02em]"
        >
          {event.title}
        </div>
      );
    ...
    case "CraftEventList": {
      return (
        <EventListCards
          ...
          cardBg={(props.cardBg as string) ?? "#ffffff"}
          cardBorder={(props.cardBorder as string) ?? "#e5e7eb"}
          titleColor={(props.titleColor as string) ?? "#111827"}
          textColor={(props.textColor as string) ?? "#6b7280"}
